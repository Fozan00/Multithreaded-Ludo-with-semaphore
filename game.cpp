...
pthread_t mainId,killerthread;			//Mainthread Id, Collision aur win check
int totaltokens=1;			//Input
int stopmaster=false;			//When 3 players complete

int previousturn=0;				//2 repetitive turns of player not possible.

pthread_t playerID1[4];
int allpassed[4];			//Tokens completed for each player
sem_t SemDice;
sem_t SemBoard;
int playing=1;				//Current Player on Dice
int playingboard=1;			//Curr Player on Board
int Diceonboard=0;			//Dice value for PLayer on Board
int window=1;
int Winningplayer[3]={1,2,3};	//index 0 first pos 1 pe 2nd 2 pe 3rd
int counter1=0;				//How many players completed
...
void* Player1thread(void* arg)
{
	...
	bool ThreeSixes=0;
		...
		srand(time(NULL));
		bool turnagain=1;		//Always true for first turn
		int counter=0;			//For insertting values like consective sixes
		...
		while(turnagain)				//If rolling dice generates 6 then bonus turn is provided
		{
			...
			playing=playerno+1;			//Index of thread player + 1
			diceroll = rand()%6+1;
		...
		if(!ThreeSixes)					//Do not access board if 3 sixes
		{
			...
			for(int k=0;k<3;k++)
			{
				...
				int Enterloop=1;			//If any move possible for token
				if(diceinthread[k]==6)
					Enterloop=1;
					...
					if( diceinthread[k]!=0)
					{
						...
						for(int complete=0;complete<totaltokens;complete++)
						{
							if(loc[playerno][complete]==56)
								status[playerno][complete]=0;		//finishing token status to 0
						}
				...
				if(k+1<3 && diceinthread[k+1]==0)			//Do not check value of dice if six not generated
					break;
				sleep(1);
